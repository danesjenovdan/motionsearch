...
urlpatterns = [
    ...
    url(r'^activate/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,50})/$', UserViewSet.as_view({'get': 'activate'}), name='activate'),
    url(r'^forgot-password/$', UserViewSet.as_view({'post': 'forgot'}), name='activate'),
    url(r'^change-password/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,50})/$', UserViewSet.as_view({'put': 'changePassword'}), name='activate'),
]
